...
int: install
	...
	@env INT=true EC_PROXY_PLACE=$(tmp) EC_WITHOUT_SPINNER=true TEST_ALL=true go test -v ./bin/ec -timeout 90m
	@rm -rf $(tmp)
.PHONY: int
...
int-ci: install
	...
	@env INT=true EC_PROXY_PLACE=$(tmp) EC_WITHOUT_SPINNER=true go test -v ./bin/ec -timeout 90m
